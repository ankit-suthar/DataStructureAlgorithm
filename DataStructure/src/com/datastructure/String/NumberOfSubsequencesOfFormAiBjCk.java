...
We can see that aCount for this is 3, because we can choose these possibilities: (xa,ax,aa) (x means we did not use that character).
Note also that this is independent of characters in between, i.e. the aCount of aa and ccbabbbcac are the same because both have exactly 2 a’s.
Now, adding 1 a, we now have the following new subsequences: each of the old subsequences, each of the old subsequences + the new a,
and the new letter a, alone. So a total of aCount + aCount + 1 subsequences.
Now, let’s consider bCount, the number of subsequences with some a’s and then some b’s. in ‘aab’,
we see that bCount should be 3 (axb,xab,aab) because it is just the number of ways we can choose subsequences of the first two a’s, and then b.
...
Finally, we have to count the subsequences that are made without using any other b’s, and by the logic in the last paragraph, that is just aCount.
...
public class NumberOfSubsequencesOfFormAiBjCk {
    ...
    static int getTotalSubsequences(String str){
        int aCount = 0;
        int bCount = 0;
        int cCount = 0;
    ...
    public static void main(String[] a){
        String str = "aabcc";
        System.out.println(getTotalSubsequences(str));
    }
}
//Time Complexity : O(n)